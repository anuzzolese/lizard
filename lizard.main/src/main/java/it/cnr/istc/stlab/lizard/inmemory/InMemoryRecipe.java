package it.cnr.istc.stlab.lizard.inmemory;
...
public class InMemoryRecipe implements OntologyCodeGenerationRecipe {

	private URI ontologyURI;
	private InMemoryOntologyCodeModel ontologyModel;
	
	public InMemoryRecipe(URI ontologyURI) {
		this.ontologyURI = ontologyURI;
		
		OntModel ontModel = ModelFactory.createOntologyModel(OntModelSpec.OWL_DL_MEM);
        ontModel.read(ontologyURI.toString());
        
        this.ontologyModel = new InMemoryOntologyCodeModel(ontModel);
    }
    ...
    public OntologyCodeProject generate(){
         ...
         */
        OntologyCodeInterface ontologyThingInterface = addInterface(owlThing, ontologyModel);
        System.out.println("Inter thing " + ontologyThingInterface);
        createMethods(ontologyThingInterface);
        
        /*
         * Create extentional class for owl:Thing
         */
        
        ExtentionalOntologyCodeClass thingExtentional = null;
        try {
        	thingExtentional = ontologyModel.createExtentionalOntologyClass(ontologyThingInterface);
         ...
         */
        OntologyCodeClass ontologyThingClass = addClass(owlThing, ontologyModel);
        if(thingExtentional != null) ontologyModel.addExtentionalClasses(ontologyThingClass, thingExtentional);
         ...
         */
        Map<OntResource,OntologyCodeClass> classMap = ontologyModel.getClassMap();
        Set<OntResource> ontResources = classMap.keySet();
        Set<AbstractOntologyCodeClassImpl> ontologyClasses = new HashSet<AbstractOntologyCodeClassImpl>();
        for(OntResource ontResource : ontResources){
            OntologyCodeClass ontologyClass = classMap.get(ontResource);
        ...
        JCodeModel codeModel = ontologyModel.asJCodeModel(); 
    ...
    private void visitHierarchyTree(OntClass ontClass, OntologyCodeModel ontologyModel, Set<AbstractOntologyCodeClass> ontologyClasses){
    	...
    	OntologyCodeInterface ontologyInterface = addInterface(ontClass, ontologyModel);
    	
    	ontologyInterface.createMethods();
        /*
         * Create extentional class for ontologyInterface
         */
        ExtentionalOntologyCodeClass extentionalClass = null;
        try {
        	extentionalClass = ontologyModel.createExtentionalOntologyClass(ontologyInterface);
		} catch (NotAvailableOntologyCodeEntityException e) {
			e.printStackTrace();
		}
        
        OntologyCodeClass ontologyClass = addClass(ontClass, ontologyModel);
        if(extentionalClass != null) ontologyModel.addExtentionalClasses(ontologyClass, extentionalClass);
    ...
    private void addImplementations(Set<AbstractOntologyCodeClassImpl> ontologyClasses){
        ...
        for(AbstractOntologyCodeClassImpl ontologyClass : ontologyClasses){
            ...
            OntClass ontClass = ontologyModel.asOntModel().getOntClass(ontologyClass.getOntResource().getURI());
            ...
            JMethod constructor = jClass.getConstructor(new JType[]{jClass.owner()._ref(Resource.class)});
            			...
            			if(methods != null && !methods.isEmpty()){
	            			for(AbstractOntologyCodeMethod method : methods){
	            				if(method.getMethodType() == OntologyCodeMethodType.Get){
	            					...
	            					constructor.param(jc, method.getEntityName());
	            					
	            					if(ontPropertyVar == null) {
	            						ontPropertyVar = constructor.body().decl(codeModel._ref(OntResource.class), "ontResource", codeModel.ref(ModelFactory.class).staticInvoke("createOntologyModel").invoke("createOntResource").arg(method.getOntResource().getURI()));
	            						isOntPropertyVarNullable = false;
	            					}
	            					else constructor.body().assign(ontPropertyVar, codeModel.ref(ModelFactory.class).staticInvoke("createOntologyModel").invoke("createOntResource").arg(method.getOntResource().getURI()));
	            					
	            					constructor.body().add(JExpr._super().invoke("setPropertyValue").arg(ontPropertyVar).arg(method.getRange().asJDefinedClass().dotclass()));
            			...
            			if(methods != null && !methods.isEmpty()){
	            			for(AbstractOntologyCodeMethod method : methods){
	            				if(method.getMethodType() == OntologyCodeMethodType.Get){
	            					...
	            					if(ontPropertyVar == null) ontPropertyVar = ifThenBlock.decl(codeModel._ref(OntResource.class), "ontResource", codeModel.ref(ModelFactory.class).staticInvoke("createOntologyModel").invoke("createOntResource").arg(method.getOntResource().getURI()));
	            					else ifThenBlock.assign(ontPropertyVar, codeModel.ref(ModelFactory.class).staticInvoke("createOntologyModel").invoke("createOntResource").arg(method.getOntResource().getURI()));
    ...
    private OntologyCodeInterface addInterface(OntClass ontClass, OntologyCodeModel ontologyModel){
        OntologyCodeInterface ontologyInterface = null;
        try {
        	ontologyInterface = ontologyModel.createOntologyClass(ontClass, OntologyCodeInterface.class);
        } catch (NotAvailableOntologyCodeEntityException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
        }
    ...
    */
    ...
    private void createMethods(AbstractOntologyCodeClass owner){
    	...
    	System.out.println("ont model " + owner);
        ...
        while(propIt.hasNext()){
            ...
            if(range != null){
                ...
                ontologyModel.createMethod(OntologyCodeMethodType.Get, ontProperty, owner, domain, ontologyModel.getOntologyInterface(thing));
                ontologyModel.createMethod(OntologyCodeMethodType.Set, ontProperty, owner, domain, ontologyModel.getOntologyInterface(thing));
            }
        	...
        	if(superClass.isRestriction()){
        		...
        		*/
        		if(onClass != null){
        			...
        			try {
						OntologyCodeInterface rangeClass = ontologyModel.createOntologyClass(ontologyModel.asOntModel().getOntResource(onClass), OntologyCodeInterface.class);
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            OntologyCodeGenerationRecipe codegen = new InMemoryRecipe(uri);
